...
(function () {
  ...
  var pollTimer = null;
  var tickTimer = null;
  var bar = null, card = null, overlay = null, txTitle = null, txSub = null, txTimer = null;

  var css = [
    '#dsh-lifecycle{position:fixed;left:12px;bottom:60px;z-index:900;display:flex;flex-direction:column;align-items:stretch;gap:2px;width:160px;padding:6px;border-radius:14px;background:rgba(255,255,255,.92);border:1px solid rgba(0,0,0,.08);box-shadow:0 2px 10px rgba(0,0,0,.06);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC","Hiragino Sans GB","Microsoft YaHei",sans-serif;font-size:12px;line-height:1.4;color:#1b1b1c;transition:opacity .3s ease}',
    '#dsh-lifecycle .lc-btn{display:flex;align-items:center;justify-content:flex-start;gap:6px;height:30px;padding:0 10px;border:none;border-radius:8px;background:transparent;color:#3c4043;font-size:12px;line-height:1;cursor:pointer;transition:background .16s ease,color .16s ease,transform .16s ease}',
    '#dsh-lifecycle .lc-sep{display:none}',
    '#dsh-lifecycle #dsh-quota-panel{position:static!important;right:auto!important;bottom:auto!important;left:auto!important;top:auto!important;display:block!important;flex-direction:row!important;align-items:center!important;gap:0!important;margin:0!important}',
    '#dsh-lifecycle #dsh-quota-capsule{height:26px;padding:0 10px!important;border:none!important;border-radius:13px!important;background:transparent!important;box-shadow:none!important;cursor:pointer;transition:background .15s ease;display:flex!important;align-items:center;justify-content:center;gap:5px}',
    '#dsh-lifecycle #dsh-quota-capsule:hover{background:rgba(0,0,0,.05)!important}',
    '#dsh-lifecycle #dsh-quota-card{position:fixed!important;left:12px!important;right:auto!important;bottom:160px!important;width:150px!important;max-width:calc(100vw - 24px)!important;margin-top:0!important;z-index:901!important}',
    '#dsh-lifecycle .lc-btn .lc-ico{font-size:12px;line-height:1;display:inline-block;transition:transform .2s ease}',
    '#dsh-lifecycle .lc-btn:hover{background:rgba(0,0,0,.06)}',
    ...
    '#dsh-lifecycle .lc-card{position:fixed;left:12px;bottom:172px;z-index:901;width:160px;max-width:calc(100vw - 24px);padding:16px;border:1px solid rgba(0,0,0,.1);border-radius:14px;background:#fff;box-shadow:0 12px 40px rgba(0,0,0,.18);font-size:14px;color:#1b1b1c;font-family:inherit;box-sizing:border-box}',
    '#dsh-lifecycle .lc-card-title{font-size:16px;font-weight:650;margin-bottom:8px}',
    '#dsh-lifecycle .lc-card-desc{color:#61666b;font-size:13px;line-height:1.6;margin-bottom:4px}',
    '#dsh-lifecycle .lc-card-sub{color:#868a91;font-size:12.5px;line-height:1.6;margin-bottom:16px}',
    '#dsh-lifecycle .lc-card-actions{display:flex;gap:8px;justify-content:flex-end}',
    '#dsh-lifecycle .lc-action{padding:7px 14px;border:1px solid rgba(0,0,0,.12);border-radius:8px;font-size:13.5px;line-height:1.2;cursor:pointer;background:#fff;color:#3c4043;transition:background .15s ease,color .15s ease,border-color .15s ease}',
    '#dsh-lifecycle .lc-action:hover{background:rgba(0,0,0,.05)}',
    '#dsh-lifecycle .lc-action-primary{background:#4d6bfe;border-color:#4d6bfe;color:#fff}',
    '#dsh-lifecycle .lc-action-primary:hover{background:#3f5bf0}',
    '#dsh-lifecycle .lc-action-danger{background:#d9444a;border-color:#d9444a;color:#fff}',
    '#dsh-lifecycle .lc-action-danger:hover{background:#c5353a}',
    '#dsh-lifecycle .lc-esc-hint{color:#b0b3b8;font-size:11.5px;margin-top:10px}',
    '#dsh-lifecycle .lc-overlay{position:fixed;inset:0;z-index:2147483000;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,.72);backdrop-filter:blur(9px);-webkit-backdrop-filter:blur(9px)}',
    '#dsh-lifecycle .lc-overlay .lc-inner{text-align:center;max-width:520px;padding:48px;width:100%;color:#1b1b1c}',
    '#dsh-lifecycle .lc-circles{position:relative;width:96px;height:96px;margin:0 auto 26px}',
    '#dsh-lifecycle .lc-circles .lc-ring{position:absolute;inset:0;border-radius:50%;background:radial-gradient(circle at 50% 42%,#4d6bfe 0%,#7b5bf2 100%);box-shadow:0 16px 44px rgba(77,107,254,.32);display:flex;align-items:center;justify-content:center;animation:breathe 2.1s ease-in-out infinite}',
    '#dsh-lifecycle .lc-circles .lc-ring .lc-sym{color:#fff;font-size:40px;line-height:1}',
    '#dsh-lifecycle .lc-circles .lc-glow{position:absolute;inset:-14px;border-radius:50%;background:radial-gradient(circle,rgba(77,107,254,.18),transparent 70%);animation:breatheGlow 2.1s ease-in-out infinite}',
    '#dsh-lifecycle .lc-title{font-size:24px;font-weight:650;margin:0 0 8px}',
    '#dsh-lifecycle .lc-sub{font-size:14px;color:#61666b;margin:0 0 6px;line-height:1.6}',
    '#dsh-lifecycle .lc-timer{font-size:13px;color:#b0b3b8;font-variant-numeric:tabular-nums;margin-top:22px}',
    '#dsh-lifecycle .lc-inner.lc-done .lc-ring{background:radial-gradient(circle at 50% 42%,#188038 0%,#2aa64e 100%);box-shadow:0 16px 44px rgba(24,128,56,.30);animation:none}',
    '#dsh-lifecycle .lc-inner.lc-done .lc-glow{background:radial-gradient(circle,rgba(24,128,56,.16),transparent 70%);animation:none}',
    '#dsh-lifecycle .lc-inner.lc-err .lc-ring{background:radial-gradient(circle at 50% 42%,#d9444a 0%,#ef5350 100%);box-shadow:0 16px 44px rgba(217,68,74,.28);animation:none}',
    '#dsh-lifecycle .lc-inner.lc-err .lc-glow{background:radial-gradient(circle,rgba(217,68,74,.14),transparent 70%);animation:none}',
    '#dsh-lifecycle .lc-fail-actions{display:flex;gap:10px;justify-content:center;margin-top:24px}',
    ...
    '#dsh-lifecycle .lc-hide{display:none!important}'
  ].join('');
  ...
  function openConfirm(kind) {
    ...
    card.appendChild(el('div', 'lc-card-desc',
      isStop ? '关闭后，DSH 将停止运行。' : '重启过程中页面会暂时无法使用，完成后会自动恢复。'));
    card.appendChild(el('div', 'lc-card-sub',
      isStop ? '需要使用时，可以随时重新启动。' : '当前会话和设置不会被删除。'));

    var actions = el('div', 'lc-card-actions');
    var cancel = el('button', 'lc-action', '取消');
    cancel.addEventListener('click', closeCard);
    var go = el('button', 'lc-action ' + (isStop ? 'lc-action-danger' : 'lc-action-primary'), isStop ? '关闭' : '立即重启');
  ...
  function enterFailed() {
    ...
    var logs = el('button', 'lc-action', '查看日志');
