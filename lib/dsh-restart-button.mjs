...
function dshRoot() {
  ...
  // DSH_HOME 形如 <dsh-root>/home；未设置时回退到当前工作目录
  return home ? path.dirname(home) : process.cwd();
}
...
(function () {
  ...
  var css = [
    '#dsh-lifecycle{position:fixed;left:12px;bottom:64px;z-index:900;display:flex;gap:8px;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC","Hiragino Sans GB","Microsoft YaHei",sans-serif;font-size:12px;line-height:1.4;color:#1b1b1c}',
    '#dsh-lifecycle .lc-btn{display:inline-flex;align-items:center;justify-content:center;gap:4px;height:30px;padding:0 12px;border:1px solid rgba(0,0,0,.12);border-radius:8px;background:rgba(255,255,255,.88);color:#3c4043;font-size:12px;line-height:1;cursor:pointer;box-shadow:0 1px 3px rgba(0,0,0,.06);transition:background .16s ease,color .16s ease,border-color .16s ease,transform .16s ease;backdrop-filter:blur(6px);box-sizing:border-box}',
    ...
    '#dsh-lifecycle .lc-btn-stop:hover{color:#d94a4a;border-color:rgba(217,74,74,.45);background:rgba(217,74,74,.06)}',
    '#dsh-lifecycle .lc-card{position:fixed;left:12px;bottom:104px;z-index:901;width:280px;max-width:calc(100vw - 24px);padding:16px;border:1px solid rgba(0,0,0,.1);border-radius:14px;background:#fff;box-shadow:0 12px 40px rgba(0,0,0,.18);font-size:14px;color:#1b1b1c;font-family:inherit;box-sizing:border-box}',
