...
- **界面零技术名词**。
- **左下角统一控制栏** —— 若同时安装可选的 [dsh-quota-panel](https://github.com/brittanistrehlowll-oss/dsh-quota-panel) 插件，其额度胶囊（¥ 余额 ● 用量%）会被自动并入左下角的同一控制栏，与「重启 / 关闭」按钮用细分隔线隔开——两个角落的元素合并为一个整体控制区。
...
- **生命周期控制栏**：左下角（设置按钮上方）一个胶囊形控制栏，从左到右依次为：额度胶囊（若安装了 `dsh-quota-panel`）、细分隔线、⟳ **重启** / ⏻ **关闭** 按钮。点击额度胶囊会在控制栏正上方展开详情卡片。
- **重启**：点击 ⟳ **重启** 按钮，确认。页面进入接管层，新实例就绪后自动重新加载。
- **关闭**：点击 ⏻ **关闭** 按钮，确认。DSH 停止，浏览器跳转到 `http://127.0.0.1:3081/`。
- **启动**：在启动页点击 **启动 DSH**。完成后自动回到关闭前的页面（记录于 `localStorage.dsh_lastUrl`）。
- **额度**：点击胶囊（¥ 余额 ● 用量%）展开/收起控制栏上方的额度卡片；卡片会始终锚定在左下角控制栏上方。
